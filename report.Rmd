...
The dataset from the 'Prison population by type of custody, age group, and sex' section includes various custody groups such as remand (held until trial), intermediate sentence (awaiting public safety clearance), fine defaulter (imprisoned for not paying fines), and recalls (returned to prison for violating parole). 
...
Models 4, 5, and 7 performed best. Model 5 improved fit by ~27% over Model 4 due to adding Remand_pop. Model 7 swapped the response (Total_pop) and explanatory (Remand_pop) variables to better assess Remand_pop's impact. Models 1–3 reduced explanatory variables, lowering R^2 and fit, while Models 6 and 8 mirrored 7 but with similar reductions.

Linear models were evaluated using 5-fold cross-validation with RMSE and MAE. Models with different response variables were not directly comparable.
