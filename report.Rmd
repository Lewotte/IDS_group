...
We have picked some major worldwide events to see whether if there is a noticeable correlation between them and changes in the prison population.
...
We created an indicator variable `lockdown_happens` which takes the value of 1 during the first lockdown period (2020 March 26 to July 4), and 0 at any other given time. This will be used as an explanatory variable for our model.
...
Applying Occam's Razor, the simplest model that still provides meaningful explanatory power should be used. Since the r-squared of Model 3 is the highest and simplest, this makes it the most interpretable for directly assessing the lockdown’s effect without accounting for temporal trends or interactions. 
   
The lower Adjusted R-squared values across all models suggest that other unaccounted factors might also influence changes in the prisoner population, such as policy changes, release rates, or crime trends. A scan at the plots we immediately can see fluctuations that makes the dent lockdown left hard to discern from the rest. These fluctuations could be caused by many unnacounted factors, and other events, which makes it hard to obtain control data (i.e. what would the prison population look like in 2020-21 if COVID-19 did not happen).
