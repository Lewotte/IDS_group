...
PR_quarterly <- PR_quarterly %>%
  mutate(event_occurred = if_else(quarter(Date, with_year = TRUE) %in% event_quarters, 1, 0),
         event_occurred_year = if_else(year(Date) %in% floor(event_quarters), 1, 0))

#t test (messures mean before and after events)
PR_t_test_1 <- t.test(Total_pop ~ event_occurred, data = PR_quarterly) %>%
  tidy()
print(PR_t_test_1)

PR_t_test_2 <- t.test(Total_pop ~ event_occurred_year, data = PR_quarterly) %>%
  tidy()
print(PR_t_test_2)
...
#logistic regression model with Date and Event indicator for quarters
...
PR_model_2 <- linear_reg() %>%
  set_engine("glm") %>%
  fit(Total_pop ~ Date_num + event_occurred_year, data = PR_quarterly) %>%
  tidy()
print(PR_model_2)

PR_model_3 <- linear_reg() %>%
  set_engine("glm") %>%
  fit(Total_pop ~ Date_num + event_occurred + event_occurred_year, data = PR_quarterly) %>%
  tidy()
print(PR_model_3)

#time series analysis (can be used to see the difference in predicted vs actual)
PR_ts_data <- ts(PR_quarterly$Total_pop, frequency = 4)  #number of quarters in a year
PR_fit_arima <- auto.arima(PR_ts_data)
PR_forecast_result <- forecast(PR_fit_arima, h = 22)  #forecasting the next 22 periods
print(PR_forecast_result)

#clean ts data
PR_forecast_df <- rownames_to_column(as.data.frame(PR_forecast_result), var = "Date") %>%
  mutate(Date = as.Date(as.yearqtr(Date, format = "%y Q%q"), frac = 1)) #end of quarter

PR_ts_vs_real <- inner_join(PR_forecast_df, PR_quarterly , by = "Date")

PR_ts_vs_real %>%
  filter(Date <= as.Date("2016-06-01")) %>%
  ggplot(aes(x = Date)) +
  geom_line(aes(y = `Point Forecast`, color = "Point Forecast")) +
  geom_line(aes(y = `Hi 80`, color = "Hi 80")) +
  geom_line(aes(y = `Hi 95`, color = "Hi 95")) +
  geom_line(aes(y = Total_pop, color = "Total_pop")) +
  scale_color_manual(values = c("Point Forecast" = "blue", "Hi 80" = "green", "Hi 95" = "orange", "Total_pop" = "red")) +
