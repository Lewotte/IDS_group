...
Custody_PD_sliced <- Custody_PD %>%
  ...
  mutate(perc_count_gender = Count/sum(Count, na.rm = TRUE),
         ...
         d1_perc_Count_min = mean(d1_perc_Count, na.rm = TRUE) - 1*sd(d1_perc_Count, na.rm = TRUE),
         d1_perc_Count_max = mean(d1_perc_Count, na.rm = TRUE) + 1*sd(d1_perc_Count, na.rm = TRUE),
...
custody_plot_d1 <- ggplot(data = Custody_PD_sliced,
  ...
  facet_wrap(
    Age ~ Custody,
    nrow = 3,
    # vars(Age), vars(Custody),
    scales = "free_y"
             ) +
  scale_color_manual(values = c(Male = "gold",Female = "mediumorchid")) +
  labs(x = "Time", y = "Population percentage") +
  geom_blank(aes(y = d1_Count_min)) +
  geom_blank(aes(y = d1_Count_max)) +
  theme(text = element_text(size = 10))
  
print(custody_plot_d1)

  #in = 100 pixels. Keep commented unless need to export plot
  ggsave("img/custody_plot_gender_d1.jpg", plot = custody_plot_d1, width = 46, height = 7, units = "in")
...
```{r Type of Crime 2,fig.width=10, fig.height=5}
#Analyzing about Brexit
Brexit_PD <- Annual_Data_A1_7 %>%
  filter(Date >= as.Date("2016-01-01") & Date <= as.Date("2021-12-31"))

# Plot the trend of arrests over time by group with enhanced styling
ggplot(Brexit_PD, aes(x = Date, y = Arrests, color = Group)) +
  geom_line(size = 1) +
  labs(
    title = "Trend of Prisoner Population during Brexit (2016-2021)",
    subtitle = "Showing Total, Remand, Sentenced, and Non-criminal Population",
    x = "Date",
    y = "Arrests"
  ) +
  scale_y_continuous(labels = scales::comma) +  # Use comma formatting for y-axis
  scale_color_manual(values = c("Total" = "blue", "Remand" = "red", 
                                "Sentenced" = "green", "Non-criminal prisoners" = "purple")) +
  ...
  facet_wrap(~ Group, scales = "free_y")  # Allows each facet to have a different y-axis range if needed
