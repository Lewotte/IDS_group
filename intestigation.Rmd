...
Custody_PD_sliced <- Custody_PD %>%
  ...
  filter(
    year >= 2015,
    Custody == "Non-criminal prisoners" |
    Custody == "Length not recorded"
    ) %>%
  
  #grouping for plot visuals
  group_by(Custody, Gender) %>%
  mutate(perc_count_gender = Count/sum(Count, na.rm = TRUE),
         d1_perc_Count = diff(c(NA,perc_count_gender))
         # d1_Count_min = mean(d1_Count, na.rm = TRUE) - 2*sd(d1_Count, na.rm = TRUE),
         # d1_Count_max = mean(d1_Count, na.rm = TRUE) + 2*sd(d1_Count, na.rm = TRUE)
         )
...
custody_plot <- ggplot(data = Custody_PD_sliced,
  ...
  scale_color_manual(values = c(Male = "#05c5fa",Female = "#de4570")) +
  labs(x = "Time", y = "Population percentage") +
  theme(text = element_text(size = 10))
  
# print(custody_plot)

custody_plot_d1 <- ggplot(data = Custody_PD_sliced,
...
# print(custody_plot_d1)

  #in = 100 pixels. Keep commented unless need to export plot
export.width <- 12 
ggsave("img/custody_plot_noReaction.jpg", plot = custody_plot_d1, width = export.width, height = export.width, units = "in")
...
Annual_Data_A1_5ii <- Annual_Data_A1_5ii %>%
  slice(1,16,17,18,22,25)
Annual_Data_A1_5i <- Annual_Data_A1_5i %>%
  slice(1,61,62,89,128)
...
PR_t_test <- t.test(Total_pop ~ event_occurred, data = PR_quarterly) %>%
  tidy()
print(PR_t_test)

#t test is mostly not visualisable so this can kinda be used as a visual representation
ggplot(PR_quarterly, aes(x = as.factor(event_occurred), y = Total_pop, fill = as.factor(event_occurred))) +
  geom_boxplot() +
...
ts_data <- ts(PR_quarterly$Total_pop, frequency = 4) 

#number of quarters in a year
PR_quarterly_to_2012 <- PR_quarterly %>%
  filter(Date >= as.Date("2010-04-01") & Date <= as.Date("2012-06-30"))
ts_data <- ts(PR_quarterly_to_2012$Total_pop,  start = c(2010, 2), frequency = 4)  #number of quarters in a year
fit_arima <- auto.arima(ts_data)
forecast_result <- forecast(fit_arima, h = 22) 

#forecasting the next 22 periods
print(forecast_result)

#clean ts data
forecast_df <- rownames_to_column(as.data.frame(forecast_result), var = "Date") %>%
  mutate(Date = as.Date(as.yearqtr(Date, format = "%Y Q%q"), frac = 1)) #end of quarter
ts_vs_real <- inner_join(forecast_df, PR_quarterly , by = "Date")
...
ts_vs_real %>%
  ...
  scale_color_manual(values = c("Point Forecast" = "blue", "Lo 95" = "orange", "Hi 95" = "orange", "Total_pop" = "red")) +
...
#Same as Marcel's with new dates
# Adjust event dates to represent Cameron and May's time in office
event_dates <- as.Date(c("2010-05-11", "2016-07-13", "2016-07-13", "2019-07-24"))
event_quarters <- quarter(event_dates, with_year = TRUE)

# Filter data based on the relevant timeframe for Cameron and May's tenures
PR_quarterly <- PR_quarterly %>%
  filter(Date >= as.Date("2010-05-11") & Date <= as.Date("2019-07-24"))

# Checking if events occurred during the specified periods (David Cameron and Theresa May's tenure)
PR_quarterly <- PR_quarterly %>%
  mutate(event_occurred = if_else(quarter(Date, with_year = TRUE) %in% event_quarters, 1, 0))

# t-test to measure the mean before and after the events (Cameron and May's tenure)
PR_t_test <- t.test(Total_pop ~ event_occurred, data = PR_quarterly) %>%
  tidy()
print(PR_t_test)
...
ggplot(PR_quarterly, aes(x = as.factor(event_occurred), y = Total_pop, fill = as.factor(event_occurred))) +
  geom_boxplot() +
  labs(title = "Boxplot of Total Population by Event Occurrence (Cameron & May)",
       x = "Event Occurred",
       y = "Total Population") +
  theme_minimal()
...
# Time series analysis (forecasting the next 22 periods)
ts_data <- ts(PR_quarterly$Total_pop, frequency = 4)

# Filtering for relevant time periods (2010-2012)
PR_quarterly_to_tenures <- PR_quarterly %>%
  filter(Date >= as.Date("2010-05-11") & Date <= as.Date("2019-07-24"))
ts_data <- ts(PR_quarterly_to_2012$Total_pop, start = c(2010, 2), frequency = 4) 
...
# Clean forecast data for plotting
forecast_df <- rownames_to_column(as.data.frame(forecast_result), var = "Date") %>%
  mutate(Date = as.Date(as.yearqtr(Date, format = "%Y Q%q"), frac = 1))  # End of quarter

# Merge forecast data with actual data for comparison
ts_vs_real <- inner_join(forecast_df, PR_quarterly, by = "Date")
...
ts_vs_real %>%
  ...
  geom_line(aes(y = Total_pop, color = "Total_pop")) +
  scale_color_manual(values = c("Point Forecast" = "blue", "Lo 95" = "orange", "Hi 95" = "orange", "Total_pop" = "red")) +
  labs(title = "Forecast vs Real for Cameron & May's Tenure",
       x = "Date", 
       y = "Population")
