...
#event dates
event_dates <- as.Date(c("2019-07-24"))
event_quarters <- quarter(event_dates, with_year = TRUE)

#checking if in same quarter
PM_quarterly <- PM_quarterly %>%
  mutate(event_occurred = if_else(quarter(Date, with_year = TRUE) %in% event_quarters, "Theresa", "Cameron"))

#t test (measures mean before and after events)
PM_t_test <- t.test(Total_pop ~ event_occurred, data = PM_quarterly) %>%
  tidy()
print(PM_t_test)

#t test is mostly not visualisable so this can kinda be used as a visual representation
ggplot(PM_quarterly, aes(x = as.factor(event_occurred), y = Total_pop, fill = as.factor(event_occurred))) +
  geom_boxplot() +
  labs(title = "Boxplot of Total Population by Event Occurrence",
       x = "Event Occurred",
       y = "Total Population") +
...
#time series analysis (can be used to see the difference in predicted vs actual)
ts_data <- ts(PM_quarterly$Total_pop, frequency = 4) 

#number of quarters in a year
PM_quarterly_to_2012 <- PM_quarterly %>%
  filter(Date >= as.Date("2010-04-01") & Date <= as.Date("2016-07-13"))
ts_data <- ts(PM_quarterly_to_2012$Total_pop,  start = c(2010, 2), frequency = 4)  #number of quarters in a year
fit_arima <- auto.arima(ts_data)
...
forecast_df <- rownames_to_column(as.data.frame(forecast_result), var = "Date") %>%
  mutate(Date = as.Date(as.yearqtr(Date, format = "%Y Q%q"), frac = 1)) #end of quarter
ts_vs_real <- inner_join(forecast_df, PM_quarterly , by = "Date")
